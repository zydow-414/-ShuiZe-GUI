...
for _name in ["Mapping", "MutableMapping", "Sequence", "MutableSet", "Iterable"]:
...
def safe_call(stage_name, func, default_value):
    try:
        return func()
    except Exception as e:
        cprint('[{}] skipped: {}'.format(stage_name, e), 'yellow')
...
# 获取子域名
def run_subdomain():
    fast_test_mode = os.environ.get('SHUIZE_FAST_TEST') == '1'
    # 弃用 1. lijiji
    ...
    # 0. beian2NewDomain
    safe_call('beian2NewDomain', beian2NewDomain, None)
    ...
    if fast_test_mode:
        ksubdomains = []
    elif not isPanAnalysis and ksubdomain:
        # 0. 调用kSubdomain脚本
        ksubdomains = safe_call('ksubdomain', callKsubdomain, [])
    else:
        ksubdomains = []
    ...
    othersApiTotalSubdomains = [] if fast_test_mode else safe_call('othersApiSubdomain', othersApiSubdomain, [])
    ...
    # 3. github
    githubApiSubdomains = [] if fast_test_mode else safe_call('githubApiSubdomain', githubApiSubdomain, [])
    ...
    # 4. 爬虫(百度｜必应)
    spiderSubdomains = [] if fast_test_mode else safe_call('SpiderSubdomain', SpiderSubdomain, [])
...
'''
    ...
    # 5. 爬证书
    certsSubdomains = [] if fast_test_mode else safe_call('crawlCerts', lambda: crawlCerts(subdomains), [])
    print('[total: {}] Certs: {}'.format(len(certsSubdomains), certsSubdomains))
    subdomains = printGetNewSubdomains(subdomains, certsSubdomains)

    # 6. 爬友链
    fcSubdomains = [] if fast_test_mode else safe_call('FriendChinsSubdomain', lambda: FriendChinsSubdomain(subdomains), [])
    ...
    Subdomains_ips, CDNSubdomainsDict, notCDNSubdomains = safe_call(
        'checkCDN_queryA_subdomains',
        lambda: checkCDN_queryA_subdomains(Subdomains_ips, subdomains),
        ({}, {}, [])
    )

    # host碰撞,censysIPS是censys api得到的解析的IP
    censysIPS = [] if fast_test_mode else safe_call('hostCollide', lambda: hostCollide(Subdomains_ips), [])

    # 8. 获取所有子域名的参数链接（存活）
    param_Links = [] if fast_test_mode else safe_call('run_ParamLinks', run_ParamLinks, [])

    # 获取C段的IP
    CIP_List = safe_call('get_CIP', lambda: get_CIP(Subdomains_ips, CDNSubdomainsDict, censysIPS), [])
    print('C段的IP:{}'.format(CIP_List))

    # 8. 跑C段
    if not fast_test_mode:
        safe_call('run_cSubnet', lambda: run_cSubnet(CIP_List, Subdomains_ips, notCDNSubdomains, param_Links), None)
...
def launch_gui():
    from shuize_gui import main as gui_main
    raise SystemExit(gui_main())


def should_launch_gui_on_startup(argv: list[str]) -> bool:
    cli_args = argv[1:]
    if '--backend-run' in cli_args:
        return False
    if '--gui' in cli_args:
        return True
    if not getattr(sys, 'frozen', False):
        return False
    return len(cli_args) == 0


def write_startup_decision_marker(argv: list[str], launch_gui: bool) -> None:
    marker_path = os.environ.get('SHUIZE_STARTUP_DECISION_MARKER')
    if not marker_path:
        return
    try:
...
def _init():
    ...
    parse.add_option('--gui', dest='gui', action='store_true', default=False, help='launch desktop GUI')
    parse.add_option('--backend-run', dest='backendRun', action='store_true', default=False, help='internal flag for GUI-spawned backend execution')
...
if __name__ == '__main__':
    launch_gui_on_startup = should_launch_gui_on_startup(sys.argv)
    write_startup_decision_marker(sys.argv, launch_gui_on_startup)
    if launch_gui_on_startup:
        launch_gui()
    _init()
