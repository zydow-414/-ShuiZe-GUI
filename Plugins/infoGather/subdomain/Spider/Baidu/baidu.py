...
def safe_print(message):
    ...
    print(str(message).encode(encoding, errors='backslashreplace').decode(encoding, errors='replace'))
...
class BaiduSpider:
    ...
    def location(self, each_wd, baiduLink, title):
        try:
            ...
            safe_print('[{}] {} {}'.format(each_wd, link, title))
            return link
        except Exception as e:
            safe_print('[{}] {} '.format(each_wd, e.args))
            return ''

    def get_subdomain(self, each_wd, i):
        for page in range(1, self.PAGES+1):
            wd = 'site:{} {}'.format(self.domain, each_wd)
            safe_print('[{}] -> [page: {}]'.format(wd, page))
            wd = quote(wd)
    ...
    def run_link(self, Subdomains_ips):
        ...
        for each_wd in self.links:
            safe_print('[{}] : {}'.format(each_wd, self.links[each_wd]))
